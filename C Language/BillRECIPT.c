...
char * replaceWord(const char * str, const char * oldWord, const char * newWord)
{
    ...
    for (i = 0; str[i] !='\0'; i++)
    {
        if (strstr(&str[i], oldWord) == &str[i])             //in strstr ,it gives string from first address to null,and if 
        {                                                       // //second words address found it give address from that word beggining to null
            count++;

            // Jump over this word
            i = i + oldWordLength - 1;                      //++ occured so
        }
    }

    // Making a new string to fit in the replaced words
    resultString = (char *)malloc(i + count * (newWordLength - oldWordLength) + 1); //new string space reserv
    ...
    while (*str)
    {
        ...
        if(strstr(str, oldWord) == str)
        {
            ...
            i += newWordLength;         //input string is forward by new word length
            str += oldWordLength;   //input string is forward by new word length
        }
        else{
            resultString[i] = *str;
            i += 1;
